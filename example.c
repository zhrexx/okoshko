...
int main() {
    ...
    oko_Button btn1 = oko_button_create(300, 250, 200, 50, "Click to start Timer", f);
    ...
    while (oko_is_running(win))
    {
        ...
        if (show_text >= 0)
        {
            oko_draw_text(win, oko_format("Time left: %.1f", show_text / oko_get_fps(win)), f, 315, 175,
                1.5f, 0xFFFFFFFF);
            show_text -= 1;
        }
        ...
        {
            show_text = (f32)oko_get_fps(win) * atof(input.text);
        }
