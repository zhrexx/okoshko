...
static oko_Button oko_button_create(i32 x, i32 y, i32 w, i32 h,
                                    const char* text, oko_Font* font);
...
static void oko_button_draw(oko_Window* win, oko_Button* btn) {
    ...
    switch (btn->state)
    {
    case OKO_BUTTON_HOVER:
        color = btn->color_hover;
        break;
    case OKO_BUTTON_PRESSED:
        color = btn->color_pressed;
        break;
    default:
        color = btn->color_idle;
        break;
    }
    ...
    if (btn->text && btn->font)
    {
        ...
        oko_draw_text(win, btn->text, btn->font, text_x, text_y, btn->text_scale,
                      btn->text_color);
    }
}
#endif
