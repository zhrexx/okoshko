...
static void oko_input_update(oko_Window *win, oko_Input *input) {
    ...
    u8 inside = (mx >= input->bounds.x && mx < input->bounds.x + input->bounds.w &&
        ...
        if (inside) {
            ...
            if (click_offset <= 0) {
                ...
                for (i32 i = 0; i < input->text_length; i++) {
                    ...
                    i32 char_width = glyph ? (i32)(glyph->advance * input->text_scale) : (i32)(input->font->size * 0.5f * input->text_scale);
...
static void oko_input_draw(oko_Window *win, oko_Input *input) {
    u32 bg_color = (input->state == OKO_INPUT_FOCUSED) ? input->color_focused : input->color_idle;

    oko_fill_rect(win, input->bounds, bg_color);

    if (input->state == OKO_INPUT_FOCUSED) {
        oko_draw_rect(win, input->bounds, 0xFFFFFFFF);
        oko_Rect inner = {input->bounds.x + 1, input->bounds.y + 1,
    ...
    if (input->text_length == 0 && input->placeholder && input->state == OKO_INPUT_IDLE) {
    ...
    if (input->state == OKO_INPUT_FOCUSED && input->cursor_visible) {
        ...
        for (i32 i = 0; i < input->cursor_pos; i++) {
            ...
            cursor_x += glyph ? (i32)(glyph->advance * input->text_scale) : (i32)(input->font->size * 0.5f * input->text_scale);
        }
        
        i32 cursor_height = (i32)(input->font->size * input->text_scale);
        oko_draw_line(win, cursor_x, text_y, cursor_x, text_y + cursor_height, input->cursor_color);
    }
}
